...
from apps.master_data.models import Country, Currency, Incoterm, UOM, Port, PaymentTerm, Location, PreCarriageBy
...
class Command(BaseCommand):
    ...
    # -------------------------------------------------------------------------
    # Currencies — full ISO 4217 active currency list
    # -------------------------------------------------------------------------
    def _seed_currencies(self):
        ...
        for code, name in currencies:
            _, is_new = Currency.objects.get_or_create(code=code, defaults={"name": name})
            if is_new:
                created += 1
        self.stdout.write(f"  Currencies: {created} created, {len(currencies) - created} already existed")
