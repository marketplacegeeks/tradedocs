...
Constraint #15: REJECT / REWORK / PERMANENTLY_REJECT / DISABLE require a non-empty comment.
"""
...
class WorkflowService:
    ...
    def transition(document, document_type, action, performed_by, comment=""):
        ...
        """
        # Constraint #14: DISABLE is valid only for CommercialInvoice.
        if action == DISABLE and document_type != "commercial_invoice":
            raise ValidationError(
                {"action": f"DISABLE is not a valid action for {document_type}."}
            )
    ...
    def _get_transitions(document_type):
        ...
        # packing_list and commercial_invoice tables will be added when those apps are built.
