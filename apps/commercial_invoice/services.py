...
def rebuild_ci_line_items(packing_list):
    ...
    # total_quantity is the sum of net_material_weight (the weight in the PL's
    # Material Unit) across all items with the same item_code + uom — this is
    # what rate ($ per Material Unit) and amount must multiply against. Package
    # count is tracked separately via the editable packages_kind text field.
