...
def _fmt_money(val) -> str:
    """Format a monetary value with 2 decimal places and comma separators."""
    if val is None:
        return "0.00"
    try:
...
def build_pl_story(packing_list, styles):
    ...
    _INCOTERM_VISIBLE = {
        "EXW": set(), "FCA": set(), "FOB": set(),
        "CFR": {"freight"}, "CIF": {"freight", "insurance"},
        "CPT": {"freight"}, "CIP": {"freight", "insurance"},
        "DAP": {"freight", "insurance"}, "DPU": {"freight", "insurance"},
        "DDP": {"freight", "insurance"},
    }
    try:
        ci_fin = packing_list.commercial_invoice
        if ci_fin:
            currency_obj = getattr(ci_fin, "currency", None)
            currency_code_fin = safe(getattr(currency_obj, "code", "")) if currency_obj else ""

            incoterm_obj = getattr(packing_list, "incoterms", None)
            incoterm_str = safe(getattr(incoterm_obj, "code", "")) if incoterm_obj else ""
            visible = _INCOTERM_VISIBLE.get(incoterm_str, {"freight", "insurance"})

            # Sum CI line items for FOB value
            line_items_total = Decimal("0.00")
            try:
                for li in ci_fin.line_items.all():
                    amt = getattr(li, "amount", None)
                    if amt is not None:
                        line_items_total += Decimal(str(amt))
            except Exception:
                pass
            ...
            if "freight" in visible and getattr(ci_fin, "freight", None) is not None:
                try:
                    freight_amount = Decimal(str(ci_fin.freight))
                    show_freight = True
                except Exception:
                    pass
            if "insurance" in visible and getattr(ci_fin, "insurance", None) is not None:
                try:
                    insurance_amount = Decimal(str(ci_fin.insurance))
                    show_insurance = True
                except Exception:
                    pass

            if show_freight or show_insurance:
                style_amt = ParagraphStyle(
                    "PLAmt", parent=style_text, alignment=TA_RIGHT,
                )
                ...
                breakdown_rows = [
                    ...
                    [Paragraph("FOB Value (Line Items):", style_text),
                     Paragraph(f"{currency_code_fin} {_fmt_money(line_items_total)}", style_amt)],
                ]
                if show_freight:
                    breakdown_rows.append([
                        Paragraph("Freight:", style_text),
                        Paragraph(f"{currency_code_fin} {_fmt_money(freight_amount)}", style_amt),
                    ])
                if show_insurance:
                    breakdown_rows.append([
                        Paragraph("Insurance Amount:", style_text),
                        Paragraph(f"{currency_code_fin} {_fmt_money(insurance_amount)}", style_amt),
                    ])

                breakdown_tbl = Table(breakdown_rows, colWidths=[120 * mm, 60 * mm])
                breakdown_tbl.hAlign = "LEFT"
